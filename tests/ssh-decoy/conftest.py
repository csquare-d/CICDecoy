"""
Local conftest for ssh-decoy tests.

Ensures `import metrics` resolves to `ssh-decoy/metrics.py` (not dashboard/
...
_SSHDECOY_DIR = Path(__file__).resolve().parents[2] / "ssh-decoy"

for existing in [p for p in sys.path if p == str(_SSHDECOY_DIR)]:
    sys.path.remove(existing)
sys.path.insert(0, str(_SSHDECOY_DIR))
